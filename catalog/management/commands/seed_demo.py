"""Load the Quietwork demo content: Elena's three programs, their groups and the workshop.

Dates are relative to today, so the demo always has an open group and an
upcoming workshop. Safe to run more than once.
"""
...
class Command(BaseCommand):
    help = "Create or refresh the Quietwork demo content."
    ...
    def handle(self, *args, **options):
        ...
        for order, data in enumerate(PROGRAMS):
            ...
            if not program.cohorts.filter(starts_at__gt=timezone.now()).exists():
                start = _next_weekday(today, data["weekday"], 21)
                cohort = Cohort.objects.create(
                    program=program, name=f"{start:%B} group", starts_at=_at_19(start),
                    zoom_url="https://zoom.us/j/0000000000",
                )
                cohort.generate_sessions()

            # A group already running, so the member area has something to show.
            if not program.cohorts.filter(starts_at__lte=timezone.now()).exists():
                start = _next_weekday(today - timedelta(weeks=4), data["weekday"], 0)
                cohort = Cohort.objects.create(
                    program=program, name=f"{start:%B} group", starts_at=_at_19(start),
                    zoom_url="https://zoom.us/j/0000000000", is_open=False,
                )
                cohort.generate_sessions()

        if not Workshop.objects.filter(starts_at__gt=timezone.now()).exists():
